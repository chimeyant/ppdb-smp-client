  ...
  <div class="home pa-6">
    <v-flex row class="pb-2">
      ...
      <v-subheader class="text-h6">
        <h4>DOKUMEN PENILAIAN</h4>
      </v-subheader>
    </v-flex>
    <v-flex class="pb-5 pl-7">
      <div class="grey--text mb-2">
        Seluruh Dokumen Penilaian
      </div>
    </v-flex>
            ...
            <v-tooltip color="purple draken-4" bottom>
              <template v-slot:activator="{ on }">
                <v-btn text small icon v-on="on">
                  <v-icon
                    color="white"
                    @click="$router.push({ name: 'lowongan-penilaian' })"
                    >close</v-icon
                  >
                </v-btn>
              </template>
...
export default {
  ...
  created() {
    this.setPage({
      crud: true,
      dataUrl: "api/dokumen-penilaian/" + this.$route.params.formasi_apply_id,
    });
    this.fetchRecords();
  },
  mounted() {},
